...
def printHelp():
    print('\ndnWindows version 0.1')
    print('\nDetermines dinucleotide content from sequences [and correlates it with occupancy]')
    print('\nUsage: ')
    print('\tpython dnWindows.py -fasta file [-wig file] [-out file] [options]')
    ...
    print('\nAdditional options:')
    print('\t-dn dn1[,dn2...]:\tList of comma separated dinucleotides to consider (default GC,CG,CC,GG).')
    print('\t-t threshold:\tIf set, only windows with dinucleotide percentage above wich fragments are extracted (default 75).')
...
#    print("ARGV", sys.argv)
    ...
    for k in seqs.keys():
        ...
        for i in range(0,len(s)-window,step):
            content=100*(bioseq.dnContent(s[i:(i+window)],dnlist, raw=False)/float(window))
            print(int(content))
